...
const joint_buffer = 0.1 # buffer zone in which we start applying the 
                          # position hold 
...
function torque_limiter!(index, m::CmdMsg, joint_pos::AbstractVector,
    ...
    if(m.tau != 0)
        ...
        ## lower limit 
        # if(q + v*dt + 0.5*m.tau*dt^2 - lower > joint_buffer && sign(m.tau) == -1)
        #     m.Kp = Kp 
        #     m.Kd = Kd 
        #     m.pos = lower 
        #     m.tau = 0.0
        ...
        ## upper limit 
        # if(q + v*dt + 0.5*m.tau*dt^2 - upper > -joint_buffer && sign(m.tau) == 1)
        #     m.Kp = Kp 
        #     m.Kd = Kd 
        #     m.pos = upper 
        #     m.tau = 0.0
